...
(function (global) {
  ...
  function drawMiniGraph(g, playT, st) {
    ...
    /* --- 격자 + 왼쪽 Y축 숫자 ---
       §204-B (옥대표님 2026-06-06) — 투명 chart card 위에서도 격자·축이
       선명히 보이도록 alpha 0.10→0.22, 0.07→0.18, 외곽 0.14→0.28 보강.
       Y축 라벨 폰트 weight 600 (가독성). 본인 spec §3-1 참조. */
    ...
    yTicks.forEach(function (tv) {
      ...
      ctx.strokeStyle = 'rgba(255,255,255,0.22)';   /* §204-B 0.10→0.22 */
      ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(padL, gy); ctx.lineTo(padL + plotW, gy);
      ctx.stroke();
      ctx.fillStyle = 'rgba(232,240,250,0.92)';      /* §204-B 0.62→0.92 (가독) */
      ctx.textAlign = 'right';
      ctx.fillText(tickLabel(def, tv), padL - 4, gy);
    });
    /* 세로 격자 — ±2.5초 보조선 (시간 스케일 감각) */
    ctx.strokeStyle = 'rgba(255,255,255,0.18)';      /* §204-B 0.07→0.18 */
    ctx.lineWidth = 1;
    ...
    /* 플롯 영역 외곽 — 진한 윤곽 */
    ctx.strokeStyle = 'rgba(255,255,255,0.28)';      /* §204-B 0.14→0.28 */
    ctx.lineWidth = 1;
    ...
    // 선은 흰색으로 통일한다 (§D1).
    // §204-B (옥대표님 2026-06-06) — 영상 위 가독성 위해 데이터 line 에
    // dark halo (canvas shadowBlur). 본인 spec §3-2 line halo plugin 패턴.
    var i0 = sampleIndexAtTime(R.samples, winLo);
    if (i0 > 0) i0--;
    ctx.save();
    ctx.shadowColor = 'rgba(10,14,26,0.85)';
    ctx.shadowBlur = 4;
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = 0;
    ...
    ctx.stroke();
    ctx.restore();           /* §204-B — shadowBlur (halo) restore */
  ...
  function ensureMap() {
    ...
    try {
      ...
      /* §301 (옥대표님 2026-06-06) — 라이브 dashboard 의 라이트 OSM 과
         분리해 replay viewer 는 다크 cockpit tile (CartoDB DarkMatter).
         영상 + 차트 + UI 와 톤 일관 + 영상 명도대비 일관.
         CartoDB 무료 무료 attribution 필수 (© OSM + © CARTO).
         본인 (샘 정 #2) 구현 — v2 staging 전용. */
      L.tileLayer(
