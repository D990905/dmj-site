...
(function () {
  ...
  var STATS_TIP = {
    ...
    awa: 'AWA = 겉보기 풍각(보트가 실제로 느끼는 바람 각). 진풍(CWA)에 ' +
      '보트 속도를 벡터로 합쳐 계산합니다 — 빠를수록 겉보기 바람이 앞으로 ' +
      '당겨져 AWA가 CWA보다 작아지고, 풍하로 바람보다 빠르면 정풍하에서도 ' +
      '바람이 앞에서 붑니다(윙 트림·세일 감각에 직결). 풍속을 입력해야 ' +
      '계산됩니다. AWA는 속도에 반비례하므로 \'상위 50%·20%\'는 가장 빠르게 ' +
      '활주할 때(AWA가 작은 구간)의 겉보기각을 대표로 뽑습니다 — 저속·펌핑 ' +
      '순간의 큰 각(옆에서 부는 바람)은 대표에서 제외됩니다. 풍상·풍하 모두 ' +
      '같은 기준(고속 활주 시 겉보기각)입니다.',
  ...
  function renderStatsPanel() {
    ...
    foot.push('SOG·VMG·CWA·AWA·힐·피치는 풍상/풍하 × 포트/스타보드 4 버킷으로 ' +
