...
(function (global) {
  ...
  var VPS = {
    ...
    /* §484 재보정 — 1.00 → 1.20.
       §483 이 예측기의 빠진 항력을 채우면서 예측 VMG 가 내려갔고, §484 가
       택 각도를 실측으로 원복하면서 약·중풍 예측이 더 내려갔다(12kt 배속
       13.1kt). 옥대표 실측 최고 지속창은 같은 조건에서 17~19kt 이라 모델이
       **약·중풍에서 보수적**이다. 그래서 "예측의 120% 달성 = 100점" 으로
       둔다 — 예측을 넘어서는 게 정상인 구간이 있다는 뜻이고, 그 사실을
       숨기지 않는다.
       ⚠ 근본은 속도-풍속 곡선의 **기울기**다(모델 12→20kt 13.1→21.9,
       실측 최고는 19~20 으로 거의 평평). 두 세션 다 풍속이 눈대중이라
       기울기를 맞출 근거가 없다 — 계기 풍속이 생기면 여기부터 다시 본다. */
    UPWIND_RATIO_TOP: 1.20,           // §484 (§424 1.00 → 1.20)
    DOWNWIND_RATIO_FLOOR: 0.00,       // §424 0.60→0.00: ratio≈score (백분위형)
    /* §483 재보정 (2026-09-01): 1.60 → 2.20.
       예측기에서 빠져 있던 라이더 공기저항을 넣자 풍상 예측 V_boat 가
       내려갔다. 이 항의 분모가 그 V_boat 이므로 비율이 통째로 올라가
       세 등급 점수가 8~11점씩 떠올랐다(elite 12kt 88). 분모의 의미가
       바뀌었으니 상한도 같이 옮긴다.
       ⚠ UPWIND_RATIO_TOP 은 1.00 그대로 둔다 — 이제야 그 값이 뜻을
       갖는다. 예측 VMG 가 실측(옥대표 GPS·Formula Kite 실측)에 맞춰진
       '잘 탔을 때의 목표'라서, 그걸 100% 달성 = 100점이 정직하다.
       예전에는 예측이 비현실적으로 높아 100% 달성이 애초에 불가능했다. */
    DOWNWIND_RATIO_TOP: 2.60,         // §484 2.20→2.60 (같은 이유)
  ...
  var VPS_BANDS = [
    { min: 80, tier: 'elite',        label: '매우 높음',        color: '#50AE33' },
    { min: 60, tier: 'advanced',     label: '높음',     color: '#50AE33' },
    { min: 40, tier: 'intermediate', label: '보통', color: '#F9DA4A' },
    { min: 20, tier: 'foundational', label: '낮음', color: '#D78A32' },
    { min: 0,  tier: 'learning',     label: '매우 낮음',     color: '#C23328' }
  ];
