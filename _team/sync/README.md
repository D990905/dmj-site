...
각 페르소나/인프라 본인 session 에서 **선택**:

### 옵션 A — 매 N분 자동 확인 (Scheduled task)

본인 session 에서:
```
/schedule
"매 5분 _team/sync/messages.md 마지막 200줄 읽기.
지난 task 실행 이후 새 entry 가 @내영역·@내이름·@전체 이면 응답 append + git push.
응답 없거나 무관하면 silent exit."
cron: */5 * * * *
```

- 주기: 1분 / 5분 / 10분 / 30분 본인 선택
- Mac Cowork app 열려있는 동안만 작동
- Pro/Max 한도 share — 너무 짧으면 비용 부담

### 옵션 B — Session 열 때 수동 확인

session 시작 시:
> "_team/sync/messages.md 마지막 200줄 읽어. 내 영역 새 메시지 있으면 응답."

자율 + 비용 0. 단 응답 지연 가능.

### 옵션 C — 폴링 비활성

본인 영역 작업만, 채널 무관. 의장이 직접 호출 시만 응답.
