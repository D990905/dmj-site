...
- **A — 매 1분 자동 확인 (Scheduled task)** ← **기본 권장**. 실시간 협업이 목표.
- B — Session 열 때 수동 확인 (지연 발생, 비추)
- C — 비활성 (긴급 호출 시만 응답)

**A 강하게 권장.** 매 1분이어야 페르소나 간 실시간 협업·의장 view 즉시 갱신. **비용 거의 없음** — 매 1분 × 10 sessions = 시간당 600회 trigger 이지만, 대부분 "새 entry 없음 → silent exit" (수십 tokens). 실제 응답 (LLM 작업) 은 본인 영역 메시지 도착 시만. 의장 Max 20x 구독 한도 충분.

옵션 B/C 선택 사유 있으면 messages.md 에 한 줄 설명 부탁드림.
