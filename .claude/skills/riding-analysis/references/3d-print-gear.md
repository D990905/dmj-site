...
### ⚖️ 기종 — 코덱스 교차검증 후 (2026-09-01)

코덱스는 **H2S 를 1순위**로 꼽았다. 이유는 하나, **빌드볼륨**이다.

| 기종 | 빌드볼륨 | 챔버 | 필터 | 가격 |
|---|---|---|---|---|
| P2S | 256×256×256 | 밀폐(수동), 베드 110°C | 활성탄 | 799,000 |
| **X2D** | **256×256×260** | 능동 65°C | **G3+H12 HEPA+활성탄 3단** | **1,099,000** |
| **H2S** | **340×320×340** | 능동 65°C, 노즐 350°C | ⚠ **미확인** | ~1,690,000 (US $1,249) |
| H2C | 305×320×325 | 능동 65°C | 3단 | ~3,600,000 |

**갈림길은 하나뿐이다 — 하네스 등판을 한 장으로 뽑을 것인가.**
등판이 250×120 이면 X2D 의 256×256 베드에 **수치상만 들어간다**(코덱스
표현). ASA 는 브림이 필요하고 퍼지 라인도 베드를 먹는다. 대각선 배치로
피할 수는 있다(대각 362mm).
- 등판을 한 장으로, 여러 번 시행착오하며 뽑을 계획 → **H2S**
- 케이스·풋스트랩 부품이 먼저고 등판은 나중/분할/대각 → **X2D**

⚠ **H2S 필터 사양 미확인** — 발표 기사에 언급이 없다. HEPA 가 없다면
X2D 대비 안전 이점이 사라지므로 **구매 전 반드시 확인**할 것.
...
| 항목 | 금액 |
|---|---|
| **Bambu Lab X2D** (능동챔버 + 3단 HEPA 필터 기본) | 1,099,000 |
| ASA 1kg (국산) | 26,000 |
| TPU 95A 1kg | 33,000 |
| PETG 1kg | 22,000 |
| 필라멘트 건조기 | 58,910 |
...
| **총합** | **약 148만 ~ 160만원** (X2D 기준. P2S 로 내리면 -30만이나 외부 HEPA 비용이 붙는다) |
