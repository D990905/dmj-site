...
try{const r=await fetch(new URL('data/operations-config.json',root),{cache:'no-store'});if(!r.ok)return;config=await r.json()}catch{return}
...
bar.append(text);const notice=document.createElement('a');notice.href=new URL('policy/privacy.html#visitor-analytics',root).href;notice.textContent='방문 분석 상세 안내';notice.style.display='block';notice.style.marginBottom='12px';bar.append(notice);function choice(label,value){const b=document.createElement('button');b.type='button';b.textContent=label;b.onclick=()=>{consent={allow:value,date:Date.now()};try{localStorage.setItem(key,JSON.stringify(consent))}catch{}bar.remove();if(value)start()};bar.append(b)}
...
if(!consent||typeof consent.allow!=='boolean'||!Number.isFinite(consent.date)||consent.date>Date.now()||Date.now()-consent.date>90*86400000)document.body.append(bar);else if(consent.allow)start();
function start(){if(started)return;started=true;let session;try{session=sessionStorage.getItem('dmj_visit_session');if(!session){session=crypto.randomUUID();sessionStorage.setItem('dmj_visit_session',session)}}catch{session=crypto.randomUUID()}
 const send=event=>{if(consent?.allow!==true)return;const page=location.pathname;if(page.includes('/admin/')||page.includes('profile')||page.includes('login'))return;fetch(config.visitEndpoint,{method:'POST',headers:{'Content-Type':'application/json','apikey':config.visitPublicKey},body:JSON.stringify({id:crypto.randomUUID(),session,page,event,consent:'analytics-2026-09-24'}),keepalive:true}).catch(()=>{})};
