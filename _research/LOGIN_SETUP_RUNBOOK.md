...
## 0. ✅ Supabase 깨우기 — 완료 (2026-09-04)

프로젝트가 paused 상태였다("Project 'dmj-site' is paused"). 대시보드에서
**Resume project** 를 눌러 복원했고, 약 2분 뒤 `/auth/v1/settings` 가
200 을 반환하며 살아났다. 데이터는 일시정지 시점 그대로 보존.

⚠ **다음 pause 기한은 2027-08-23** 이라고 화면이 안내했다. 그 전에 다시
7일 무활동이 생기면 또 잠긴다 — 아래 keep-alive 를 꼭 올려야 한다.

### 서버가 알려준 현재 상태 (2026-09-04)

`/auth/v1/settings` 직접 조회 결과:

| 제공자 | 상태 |
|---|---|
| **email** | ✅ 켜짐 — **지금 바로 쓸 수 있다** (커스텀 로그인) |
| kakao | ❌ 꺼짐 (§180 에 코드는 있었지만 서버 쪽은 안 켜져 있었다) |
| google | ❌ 꺼짐 |
| apple | ❌ 꺼짐 |
| naver | ❌ **목록에 아예 없음** — Supabase 기본 제공자가 아님이 확인됨 |

즉 **이메일/비밀번호 로그인은 오늘부터 작동한다.** 소셜은 아래 1·2 를
해야 켜진다.
